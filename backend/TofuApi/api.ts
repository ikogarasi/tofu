...
export class CommentClient extends ApiBase {
    ...
    getCommentsForCarrier(carrierId: string): Promise<FileResponse | null> {
        let url_ = this.baseUrl + "/api/Comment?";
        if (carrierId === undefined || carrierId === null)
            throw new Error("The parameter 'carrierId' must be defined and cannot be null.");
        else
        ...
        return this.transformOptions(options_).then(transformedOptions_ => {
            ...
            return this.processGetCommentsForCarrier(_response);
        });
    }

    protected processGetCommentsForCarrier(response: Response): Promise<FileResponse | null> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
    ...
    createNewComment(commentDTO: CommentDTO | undefined): Promise<FileResponse | null> {
        let url_ = this.baseUrl + "/api/Comment";
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.transformOptions(options_).then(transformedOptions_ => {
            ...
            return this.processCreateNewComment(_response);
        });
    }

    protected processCreateNewComment(response: Response): Promise<FileResponse | null> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200 || status === 206) {
            const contentDisposition = response.headers ? response.headers.get("content-disposition") : undefined;
